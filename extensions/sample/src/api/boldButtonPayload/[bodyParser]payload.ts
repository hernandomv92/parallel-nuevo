...
export default async function boldButtonPayload(
  request,
  response
) {
  ...
  const contextHomeUrl =
    request?.app || request?.locals
      ? (getContextValue(request, 'homeUrl', '') as string)
      : '';
  const fallbackHomeUrl =
    request.get && request.protocol
